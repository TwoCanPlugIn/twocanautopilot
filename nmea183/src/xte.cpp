...
//   ASSERT_VALID( this );
   ...
   FAAModeIndicator = wxEmptyString;
}
...
//   ASSERT_VALID( this );
   ...
   **     F = RTK Float mode M = Manual Input Mode 
   **     N = Data Not Valid 
   ...
   */
   ...
   IsDataValid = NTrue;
   if(nFields >= 6){
       wxString mode_string = sentence.Field( 6 );
       if(!mode_string.StartsWith(_T("*"))) {
          FAAModeIndicator = mode_string;
           if (FAAModeIndicator == _T("N")) {   // Not valid
               IsDataValid = NFalse;
           }
       }
   }
